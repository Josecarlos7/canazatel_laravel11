...
@if (count($deudores) != 0)
<a class="btn btn-warning btn-sm" href="{{ url('alerta/deudores/'.$sucursal->ID_SUC) }}" target="_blank"><i class="fa fa-print"></i> IMPRIMIR LISTA DE CLIENTES CON DEUDA</a>
...
<table class="table table-bordered table-striped table-hover text-center" id="table-search_{{ $sucursal->ID_SUC }}">
    ...
    <tbody>
        ...
        <tr>
            <td class="bg-{{ $deudor['alerta'] }}">{{ $index + 1 }}</td>
            <td class="bg-{{ $deudor['alerta'] }}"><span class="badge bg-black">{{ $deudor['cod_cli'] }}</span></td>
            <td class="bg-{{ $deudor['alerta'] }}">{{ $deudor['nom_cli'].' '.$deudor['ape_cli'] }}</td>
            <td class="bg-{{ $deudor['alerta'] }}">{{ $deudor['ci_cli'] }}</td>
            <td class="bg-{{ $deudor['alerta'] }}">{{ $deudor['tipo_pago'] }}</td>
            <td class="bg-{{ $deudor['alerta'] }}"><span class="badge bg-red">{{ $deudor['dias_deuda'] }} /dias</span></td>
            <td class="bg-{{ $deudor['alerta'] }}">
                <a href="{{ url('cliente/informacion/'.$deudor['id_cli']) }}" class="btn btn-info btn-sm" title="Datos del cliente"><i class="fa fa-info-circle"></i></a>
            </td>
        </tr>
        @endforeach
    </tbody>
